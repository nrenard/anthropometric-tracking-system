...
describe("useMeasurementWizard", () => {
  ...
  it("steps 2-4 advance with validation when all fields are filled", () => {
    const { result } = renderHook(() => useMeasurementWizard())
    act(() => {
      result.current.setField("weight", "70")
      result.current.next()
  ...
  it("blocks next() from step 2 when skinfolds are missing", () => {
    const { result } = renderHook(() => useMeasurementWizard())
    act(() => {
      result.current.setField("weight", "70")
      result.current.next()
    ...
    expect(result.current.step).toBe(2)
    expect(result.current.errors["skinfolds.chest"]).toBeDefined()
  })

  it("blocks next() from step 2 when a skinfold value is not positive", () => {
    const { result } = renderHook(() => useMeasurementWizard())
    act(() => {
      result.current.setField("weight", "70")
      result.current.next()
    ...
    act(() => {
      result.current.setField("skinfolds.chest", "0")
      result.current.next()
    })
    expect(result.current.step).toBe(2)
    expect(result.current.errors["skinfolds.chest"]).toBeDefined()
  ...
  it("blocks next() from step 2 when a skinfold exceeds 100", () => {
    const { result } = renderHook(() => useMeasurementWizard())
    act(() => {
      result.current.setField("weight", "70")
      result.current.next()
    ...
    act(() => {
      result.current.setField("skinfolds.chest", "101")
      result.current.next()
    })
    expect(result.current.step).toBe(2)
    expect(result.current.errors["skinfolds.chest"]).toBeDefined()
  ...
  it("blocks next() from step 3 when required perimeter fields are missing", () => {
    const { result } = renderHook(() => useMeasurementWizard())
    act(() => {
      result.current.setField("weight", "70")
      result.current.next()
    ...
    expect(result.current.step).toBe(3)
    expect(result.current.errors["perimeters.neck"]).toBeDefined()
  })

  it("blocks next() from step 4 when diameter fields are missing", () => {
    const { result } = renderHook(() => useMeasurementWizard())
    act(() => {
      result.current.setField("weight", "70")
      result.current.next()
    ...
    expect(result.current.step).toBe(4)
    expect(result.current.errors["diameters.humerus"]).toBeDefined()
  })

  it("blocks next() from step 5 when data is incomplete", () => {
    const { result } = renderHook(() => useMeasurementWizard())
    act(() => {
      result.current.setField("weight", "70")
      result.current.next()
    ...
    act(() => {
      result.current.setField("weight", "")
      result.current.next()
    })
    expect(result.current.step).toBe(5)
    expect(result.current.errors.weight).toBeDefined()
  ...
  it("valid data in each step produces no errors", () => {
    const { result } = renderHook(() => useMeasurementWizard())
